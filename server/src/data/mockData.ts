...
export interface Learner {
  ...
  email: string;
  avatar: string;
  membershipType: "basic" | "standard" | "premium" | "none";
  joinDate: string;
  trainerId?: string; // Optional - some learners don't have trainers
  gymId?: string; // Optional - some learners don't go to gym
  ...
  // Activity & Goals
  recentActivity: WorkoutActivity[];
  goals: Goal[];
...
export function getGymById(gymId: string): Gym | undefined {
...
export function getTrainersByGymId(gymId: string): Trainer[] {
  return trainers.filter(trainer => trainer.gymId === gymId);
}

export function getLearnersByTrainerId(trainerId: string): Learner[] {
  return learners.filter(learner => learner.trainerId === trainerId);
}

export function getLearnersByGymId(gymId: string): Learner[] {
  return learners.filter(learner => learner.gymId === gymId);
}

export function getLearnersWithoutTrainer(): Learner[] {
  return learners.filter(learner => !learner.trainerId);
}

export function getLearnersWithoutGym(): Learner[] {
  return learners.filter(learner => !learner.gymId);
}

export function getIndependentLearners(): Learner[] {
  return learners.filter(learner => !learner.trainerId && !learner.gymId);
}
