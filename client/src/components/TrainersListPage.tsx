...
interface TrainersListPageProps {
  onTrainerClick?: (trainerId: string) => void; // Optional for backward compatibility
}
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {trainersData.map((trainer) => (
            <Link key={trainer.id} to={`/trainers/${trainer.id}`}>
              <Card
                className="border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer group h-full"
              >
